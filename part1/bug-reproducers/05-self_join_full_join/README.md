...
The `artists` contains a single row with `id = 1`. The `WHERE artists.id <> 1` condition evaluates to `FALSE`, so no rows should be returned, but the patched binary returns a non-empty result set.