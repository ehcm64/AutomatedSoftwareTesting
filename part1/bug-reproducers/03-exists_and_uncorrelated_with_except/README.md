...
What's interesting is that the provided patched version actually returns the expected result, while the provided unmodified SQLite 3.51.1 returns the wrong result. To gather more insights, we tested the query on the official SQLite 3.53.1 version, which also returns the expected result, making us believe that the bug already existed in SQLite 3.51.1 and was fixed in a later version.