...
The `data.id` column has `TEXT` affinity, so the value `1` is stored as the string `'1'`. On the other hand, the `idmap.id` keep the affinity of the original tables, so the value `1` is stored as an integer. According to SQLite's type system rules, the comparison of `TEXT` and `INT` values doesn't lead to equality even if they represent the same number. Therefore, the query should not return the row `data.id = '1'`, but it should return instead the row `data.id = '4'`.